...
class DDPG:
    ...
    def reset(self) -> None:
        ...
        self.PolicyNetworkHealth= self.policy(n_traj=100,action_dim = self.args.n_action, traj_dim=9,hidden_dim=64,attention_head=4, state_dim=self.args.input_shape, min_bound=self.args.max_action, max_bound=self.args.min_action,traj_bound = np.array([0.1]*9))
        self.PolicyOptimizerHealth = torch.optim.Adam(self.PolicyNetwork.parameters(),lr=self.args.actor_lr)
        self.TargetPolicyNetworkHealth = self.policy(n_traj=100,action_dim = self.args.n_action, traj_dim=9,hidden_dim=64,attention_head=4, state_dim=self.args.input_shape, min_bound=self.args.max_action, max_bound=self.args.min_action,traj_bound = np.array([0.1]*9))

        self.QNetworkHealth = DDPGCritic(hidden_dim = self.args.hidden_dim)
        self.QOptimizerHealth = torch.optim.Adam(self.QNetwork.parameters(),lr=self.args.critic_lr)
        self.TargetQNetworkHealth = DDPGCritic(hidden_dim = self.args.hidden_dim)
        ...
        hard_update(self.TargetPolicyNetworkHealth,self.PolicyNetworkHealth)
        hard_update(self.TargetQNetworkHealth,self.QNetworkHealth)
    ...
    def learn(self) -> None:
        ...
        state,action,reward,next_state,done,health_reward = self.buffer.shuffle()
        ...
        health_reward = torch.Tensor(health_reward)

        # Update the Health Network
        
        target_critic_action_health = self.TargetPolicyNetworkHealth(next_state)
        target = self.TargetQNetworkHealth(next_state,target_critic_action_health)
        y = health_reward + self.args.gamma*target*(1-done)
        critic_value = self.QNetworkHealth(state,action)
        critic_loss = torch.mean(torch.square(y - critic_value),dim=1)
        self.QOptimizerHealth.zero_grad()
        critic_loss.mean().backward()
        self.QOptimizerHealth.step()

        action_health = self.PolicyNetworkHealth(state)
        critic_value = self.QNetworkHealth(state,action_health)
        actor_loss = -critic_value.mean()
        self.PolicyOptimizerHealth.zero_grad()
        actor_loss.mean().backward()
        self.PolicyOptimizerHealth.step()

        # Update the Normal Network
        ...
        if self.learning_step%3 == 0:
            actions = self.PolicyNetwork(state)
            action_health = self.PolicyNetworkHealth(state)

            loss = nn.KLDivLoss(actions,action_health) + torch.mean(torch.square(self.QNetwork(state,action) - self.QNetworkHealth(state,action)),dim=1)
            self.PolicyOptimizer.zero_grad()
            loss.mean().backward()
            self.PolicyOptimizer.step()
